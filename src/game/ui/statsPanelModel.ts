...
export function buildStatsPanelModel(run: RunState): StatsPanelModel {
  ...
  return {
    general: [
      { label: 'HEALTH', value: `${Math.ceil(run.health)} / ${Math.round(run.stats.maxHealth)}` },
      { label: 'DAMAGE', value: multiplier(run.stats.damage) },
      { label: 'ATTACK SPEED', value: multiplier(run.stats.attackSpeed) },
      { label: 'MOVE SPEED', value: `${Math.round(run.stats.moveSpeed)}` },
      { label: 'CRIT CHANCE', value: percent(run.stats.critChance) },
      { label: 'CRIT DAMAGE', value: multiplier(run.stats.critDamage) },
      { label: 'PICKUP RADIUS', value: `${Math.round(run.stats.pickupRadius)}` },
      { label: 'DASH COOLDOWN', value: seconds(run.stats.dashCooldown) },
      { label: 'BOSS DAMAGE', value: multiplier(run.stats.bossDamage) },
    ...
    weapons: [...run.weapons].map((id) => weaponDisplay(run, synergies, id)),
  };
}

function weaponDisplay(run: RunState, synergies: WeaponSynergySystem, id: WeaponId): WeaponStatDisplay {
  const definition = WEAPONS[id];
  const state = run.getWeaponState(id);
  const stats = state.stats;
  const effectiveDamage = stats.damage * run.stats.damage * synergies.damageMultiplier(id);
  const effectiveCrit = run.stats.critChance + stats.critChance + synergies.critChanceBonus(id);
  const effectiveCritDamage = run.stats.critDamage + stats.critDamage;
  const crimsonOrbit =
    id === 'bloodletter-axe' && state.level >= MAX_WEAPON_LEVEL
      ? calculateCrimsonOrbit(stats, run.stats.attackSpeed)
      : undefined;
  ...
  return {
    ...
    primary: `${Math.round(effectiveDamage * (crimsonOrbit?.damageScale ?? 1))} DMG  |  ${crimsonOrbit ? 'CONTINUOUS' : seconds(stats.cooldownMs / run.stats.attackSpeed)}  |  ${percent(effectiveCrit)} CRIT x${effectiveCritDamage.toFixed(2)}`,
