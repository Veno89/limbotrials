...
describe('enemy balance', () => {
  it('keeps fodder contact damage below specialist contact damage', () => {
    ...
    expect(ENEMIES['grave-crawler'].contactDamage).toBe(4);
    expect(ENEMIES['lost-soul'].contactDamage).toBeLessThan(ENEMIES['tormented-shade'].contactDamage);
    expect(ENEMIES['grave-crawler'].contactDamage).toBeLessThan(ENEMIES['void-caster'].contactDamage);
  });
  ...
  it('makes later replacements sturdier and more rewarding than opening fodder', () => {
    ...
    expect(ENEMIES['lantern-ghost'].maxHealth).toBeGreaterThan(ENEMIES['tormented-shade'].maxHealth);
    expect(ENEMIES['gravebound-archer'].xp).toBeGreaterThan(ENEMIES['grave-crawler'].xp);
