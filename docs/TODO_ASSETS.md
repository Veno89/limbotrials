# Hand-Crafted Asset Replacement Checklist

Here is the exhaustive checklist of everything in the game that currently lacks a dedicated, hand-drawn asset in your `assets/test/` folder.

This includes assets that are currently using AI placeholders, assets sharing a generic texture, and assets that don't have *any* unique art assigned yet.

## 🖼️ Backgrounds & UI
- [ ] **Main Menu Background** 
- [ ] **Character Select Background** 

## 🤺 Player Characters
- [x] **Haunted Base Sprite** (`assets/test/characters/haunted.png`)
- [x] **Penitent Base Sprite** (`assets/test/characters/thepenitent.png`)
- [ ] **Ashwalker Base Sprite** 

## 💀 Enemies & Bosses
*(Currently you only have `lost_soul`, `grave_crawler`, and `stalker` in the test folder)*
- [x] **Wretched Runt** 
- [x] **Limbo Knight** 
- [ ] **Tormented Shade** 
- [ ] **Limbo Abomination / Brute** 
- [ ] **Void Caster** 
- [ ] **Screamer** 
- [ ] **Flayed Wanderer** 
- [ ] **Lantern Ghost** 
- [ ] **Sentinel of Woe** 
- [ ] **Gravebound Archer** 
- [ ] **Veil Stalker** 
- [ ] **Mire Crawler** 
- [ ] **Basic Archer** 
- [ ] **Floor 1 Boss / Warden** 

## ☄️ Projectiles & Attacks
- [ ] **Void Orb** 
- [ ] **Magic Projectile** 
- [ ] **Laser Beam** 

## 🗡️ Weapons (Missing Custom Assets)
*(Currently you only have 10 custom weapon assets out of 31 weapons)*
- [x] **Poison Flask**
- [x] **Sanguine Needle**
- [x] **Spectral Chains**
- [x] **Gravetide Repeater**
- [ ] **Saintbreaker Pike**
- [ ] **Ashen Orbit**
- [ ] **Choir of Teeth**
- [ ] **Eclipse Brand**
- [ ] **Rustbound Dagger**
- [ ] **Stoning Verdict**
- [ ] **Grave Spark**
- [ ] **Bonefan**
- [ ] **Candlebrand**
- [ ] **Bellringer Mace**
- [ ] **Crowfeather Arbalest**
- [ ] **Frozen Orb**
- [ ] **Meteor Hammer**
- [ ] **Hand Cannon**
- [ ] **Infernal Blunderbuss**
- [ ] **Spike Trap**
- [ ] **Pouch of Chaos**

## 💍 Artifacts (Missing Custom Assets)
*(Currently you only have `cursed_hourglass.png` and `necrotic_hand.png` out of 50+ artifacts!)*
- [ ] **Pendant of Vigor**
- [ ] **Winged Sandals**
- [ ] **Magnet Stone**
- [ ] **Sharpened Stone**
- [ ] **Blood Vial**
- [ ] **Reinforced Buckler**
- [ ] **Hallowed Ash**
- [ ] **Vampiric Fury**
- [ ] **Soul Lantern**
- [ ] **Shadow Cloak**
- [ ] **Lucky Clover**
- [ ] **Unstable Core**
- [ ] **Spiked Collar**
- [ ] **Golden Egg**
- [ ] **Death Gaze**
- [ ] **Soul Furnace**
- [ ] **Extra Pocket**
- [ ] **Spectral Pass**
- [ ] **Ascended Crown**
- [ ] **Red Ledger**
- [ ] **Heart of the Market**
- [ ] **Crown of Second Damnation**
- [ ] **Martyr’s Ledger**
- [ ] **Black Reliquary**
- [ ] **Bell of the Hollow Host**
- [ ] **Unlit Halo**
- [ ] **Cracked Prayer Bead**
- [ ] **Soot-Stained Bandage**
- [ ] **Iron Nail Charm**
- [ ] **Grave Soil Pouch**
- [ ] **Wax Seal of Mercy**
- [ ] **Black Candle Stub**
- [ ] **Chain of Lent**
- [ ] **Crowbone Dice**
- [ ] **Ember Rosary**
- [ ] **Hollow Coin**
- [ ] **Saintless Mirror**
- [ ] **Blood-Tithe Chalice**
- [ ] **Funeral Bell Clapper**
- [ ] **Thornscript Vellum**
- [ ] **Reliquary Key**
- [ ] **Crown of Ash**
- [ ] **The Red Testament**
- [ ] **Halo of Flies**
- [ ] **Necromancer's Skull**

## 🪨 Environment & Props
- [ ] **Chained Skeleton** 
- [ ] **Rusty Cage** 
- [ ] **Dark Altar** 
- [ ] **Rubble Pile** 
- [ ] **Obsidian Brazier** 
- [ ] **Spectral Soul Lantern**
